...
class UserController extends Controller
{
    ...
    public function registerJobSeekerPost(Request $request){
	    ...
	    if ($request->hasFile('filenam')) {
		    $file = $request->file('filenam');
		    $extension = $request->file('filenam')->getClientOriginalExtension();
		    $data['filename'] = $data['name'].'-CV' .'-id-'. $count. '.' . $extension; // renameing image
		    $destinationPath = 'uploads';//its refers project/public/uploads directory
		    $upload_success = $file->move($destinationPath,  $data['filename']);
		    };
	    ...
	    User::create([
		    ...
		    'original_filename' => $request->file('filenam')->getClientOriginalName()
	    ]);

	    Upload::create([
		    'user_upload_id' =>  $count,
		    'filename' => $data['filename'],
		    'original_filename' => $request->file('filenam')->getClientOriginalName()
	    ]);
    ...
    public function changePasswordPost(Request $request)
    {
        ...
        public function applicantsExport(){
                ...
                Excel::create('applicantsExport', function($excel){
                    $excel->sheet('applicants-to-excel', function($sheet){

                        $applications = JobApplication::all();


                        $sheet->loadView('exports.applicants',  compact('applications'));
                    });
                })->export('xls');
            }      
        	...
        	public function export(){
        		...
        		Excel::create('careerProUser', function($excel){
        			$excel->sheet('Users-to-excel', function($sheet){

        				$users = User::all();


        				$sheet->loadView('exports.users',  compact('users'));
        			});
        		})->export('xls');
        	}      
